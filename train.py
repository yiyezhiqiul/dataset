...
def dynamic_collate_fn(batch, max_total_patches=16):
    ...
    # 展平所有图像块并添加到缓冲区
    for sample_images, sample_labels in batch:
    ...
    for epoch in range(epochs):
        if epoch %2 ==0 or epoch==0:
            ...
            train_loader = torch.utils.data.DataLoader(dataset=isbi_dataset,
                                                       batch_size=1,
                                                       shuffle=True,
                                                       collate_fn=lambda x: dynamic_collate_fn(x, max_total_patches=16))
        # 初始化损失和正确预测的数量
        running_loss = 0.0
...
if __name__ == "__main__":
    ...
    data_path = "/media/totem_disk/totem/huang/PANDA"  #  修改为你本地的数据集位置
    print("进度条出现卡着不动不是程序问题，是他正在计算，请耐心等待")
    train_net(net, device, data_path,out_path, epochs=100, batch_size=1,classes=classes)